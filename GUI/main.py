...
import time
Window = Tk()
# Button(Window, text="close", command=exit).pack()
Label(Window, text="Welcome to RPS installation", font=("Times New Roman", 35)).grid(column=0, row=0)
def win2():
    def install():
        file = open("RPS.py", "w")
...
time.sleep(10.05)''')
        file.close()
    win = Toplevel()
    Label(win, text='''Warning!
    You need to install python from python.org/downloads to run this application!''').grid(row=0, column=0)
    Button(win, text="Install", command=install).grid(row=1, column=0)

Button(Window, text="Next", command=win2).grid(column=0, row=1)
Window.mainloop()
